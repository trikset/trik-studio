...
inline ParserRef<TokenType> operator>>(TokenType token, const SemanticAction &semanticAction)
{
	typedef typename std::conditional<utils::function_traits<SemanticAction>::arity == 0,
		SimpleParser<TokenType, SemanticAction>, TokenParser<TokenType, SemanticAction>>::type Parser;
...
inline ParserRef<TokenType> operator&(const TokenType &token, const ParserRef<TokenType> &b)
{
	return ParserRef<TokenType>(new ConcatenationParser<TokenType>(
		token >> [](Token<TokenType> const &token) { return wrap(new TemporaryToken<TokenType>(token)); }, b));
}
...
{
	return ParserRef<TokenType>(new ConcatenationParser<TokenType>(a,
		token >> [](Token<TokenType> const &token) { return wrap(new TemporaryToken<TokenType>(token)); }));
}
