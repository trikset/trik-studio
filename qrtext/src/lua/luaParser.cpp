...
QSharedPointer<ParserInterface<LuaTokenTypes>> LuaParser::grammar()
{
	...
	auto identifier = LuaTokenTypes::identifier >> [](Token<LuaTokenTypes> const &token) {
		return wrap(new ast::Identifier(token.lexeme()));
	} /= "identifier";

	// block ::= {stat}
	auto block = *stat >> [](QSharedPointer<TemporaryList> const &statList) {
	...
	stat <<= (-LuaTokenTypes::semicolon | (explist & ~(-LuaTokenTypes::equals & explist))) >>
		...
		if (!pair) {
			context().reportInternalError(
				QObject::tr("node in 'stat' semantic action is of unexpected type"));
			return wrap(new TemporaryErrorNode());
		}
		...
		if (variables.size() != values.size()) {
			context().reportError(pair, QObject::tr("Number of variables in assignment shall be equal to "
	...
	// explist ::= exp(0) {‘,’ exp(0)}
	explist <<= (exp & *(-LuaTokenTypes::comma & exp)) >> [](QSharedPointer<TemporaryPair> const &node) {
	...
	primary <<=
		...
		| LuaTokenTypes::string >>
			  [](Token<LuaTokenTypes> const &token) {
		...
		| (unop
			  & ParserRef<LuaTokenTypes>(new ExpressionParser<LuaTokenTypes>(precedenceTable,
				  LuaTokenTypes::minus, primary, binop)))
	...
	prefixexp <<= (prefixterm & *(functioncallpart | varpart)) >> [](QSharedPointer<TemporaryPair> const &node) {
		...
		for (const auto &part : as<TemporaryList>(node->right())->list()) {
			if (part->is<ast::Expression>()) {
				...
				result = QSharedPointer<ast::Expression>(new ast::FunctionCall(result,
					as<ast::Expression>(as<TemporaryList>(part)->list())));
			}
		}
	...
	prefixterm <<= identifier | (-LuaTokenTypes::openingBracket & exp & -LuaTokenTypes::closingBracket) /=
		"prefixterm";

	// args ::= ‘(’ [explist] ‘)’ | tableconstructor | String
	args <<= ((-LuaTokenTypes::openingBracket & ~explist & -LuaTokenTypes::closingBracket) | tableconstructor
	...
	tableconstructor <<= (-LuaTokenTypes::openingCurlyBracket & ~fieldlist & -LuaTokenTypes::closingCurlyBracket) >>
		...
		if (fieldList->is<TemporaryDiscardableNode>()) {
			...
			if (!list) {
				context().reportInternalError(
					QObject::tr("In 'table constructor' semantic action fieldList is "
						    "of incorrect type"));
				return wrap(new TemporaryErrorNode());
			}
	...
	fieldlist <<= (field & *(-fieldsep & field) & -~fieldsep) >> [](QSharedPointer<TemporaryPair> const &node) {
	...
	field <<= (-LuaTokenTypes::openingSquareBracket & exp & -LuaTokenTypes::closingSquareBracket
	...
	} | (exp & ~(-LuaTokenTypes::equals & exp)) >> [this](QSharedPointer<ast::Node> const &node) {
	...
	// fieldsep ::= ‘,’ | ‘;’
	fieldsep <<= -LuaTokenTypes::comma | -LuaTokenTypes::semicolon /= "fieldsep";
	...
	binop <<= LuaTokenTypes::plus >> [] { return wrap(new ast::Addition()); }
	          ...
	          | LuaTokenTypes::doubleVerticalLine >> [] { return wrap(new ast::LogicalOr()); } /= "binop";
	...
	unop <<= LuaTokenTypes::minus >> [] { return wrap(new ast::UnaryMinus()); }
	         ...
	         | LuaTokenTypes::tilda >> [] { return wrap(new ast::BitwiseNegation()); } /= "unop";

	QList<ParserRef<LuaTokenTypes>> helper {exp, prefixexp, args, stat, explist, primary, varpart, functioncallpart,
		prefixterm, tableconstructor, fieldlist, field, fieldsep, binop, unop};
