...
#include "stm32DeviceVariables.h"

#include <QtCore/QMetaClassInfo>

using namespace stm32::parts;

QString Stm32DeviceVariables::variableTemplatePath(const kitBase::robotModel::DeviceInfo &device
		, const kitBase::robotModel::PortInfo &port) const
{
	if (device.name() == "stm32LineSensor"
			|| device.name() == "stm32ObjectSensor"
			|| device.name() == "stm32ColorSensor")
	{
