...
namespace {
Binding::ConverterInterface *createConverter(const QString &type, const QString &dynamicPropertyValue,
...
SubprogramCallGenerator::SubprogramCallGenerator(const qrRepo::RepoApi &repo,
	...
	: BindingGenerator(repo, customizer, id, "subprograms/subprogramCall.t",
		  QList<Binding *>() << Binding::createConverting("@@NAME@@", "name",
		  ...
		  parent)
{
	...
	for (auto property = properties.firstChildElement("property"); !property.isNull();
