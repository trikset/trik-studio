...
TrikCppControlFlowValidator::TrikCppControlFlowValidator(const qrRepo::RepoApi &repo
...
bool TrikCppControlFlowValidator::validate(const qReal::Id &diagramId, const QString &threadId)
{
	mDiagram = diagramId;
	findInitialNode();
...
PrimaryControlFlowValidator *TrikCppControlFlowValidator::clone()
{
	return new TrikCppControlFlowValidator(mRepo, mErrorReporter, mCustomizer, parent());
}
