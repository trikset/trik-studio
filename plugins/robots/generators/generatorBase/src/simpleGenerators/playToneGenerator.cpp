...
PlayToneGenerator::PlayToneGenerator(const qrRepo::RepoApi &repo, GeneratorCustomizer &customizer, const Id &id,
	QObject *parent)
	: BindingGenerator(repo, customizer, id, "playTone.t",
		  QList<Binding *>() << Binding::createConverting("@@FREQUENCY@@", "Frequency",
			  customizer.factory()->intPropertyConverter(id, "Frequency"))
				     ...
				     << Binding::createConverting("@@WAIT_FOR_COMPLETION@@", "WaitForCompletion",
						customizer.factory()->boolPropertyConverter(id, "WaitForCompletion",
							false)),
		  parent)
{
}
