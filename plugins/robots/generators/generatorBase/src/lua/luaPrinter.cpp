...
LuaPrinter::LuaPrinter(const QStringList &pathsToTemplates, const qrtext::LanguageToolboxInterface &textLanguage,
...
void LuaPrinter::processTemplate(const QSharedPointer<qrtext::lua::ast::Node> &node, const QString &templateFileName,
	...
	pushResult(node,
		readTemplate(templateFileName)
			.replace("@@LEFT@@", popResult(node->leftOperand(), needBrackets(node, node->leftOperand(),
										    qrtext::core::Associativity::left)))
			.replace("@@RIGHT@@",
				popResult(node->rightOperand(),
					needBrackets(node, node->rightOperand(), qrtext::core::Associativity::right))));
}

bool LuaPrinter::needBrackets(const QSharedPointer<qrtext::lua::ast::Node> &parent,
	const QSharedPointer<qrtext::lua::ast::Node> &child, qrtext::core::Associativity childAssociativity) const
{
	...
	return parentPrecedence > childPrecedence
	       || (parentPrecedence == childPrecedence && mPrecedenceTable.associativity(parent) != childAssociativity);
}
	...
	const QSharedPointer<qrtext::core::ast::Node> &)
{
	const QString templatePath =
		node->key().data() ? "explicitKeyFieldInitialization.t" : "implicitKeyFieldInitialization.t";
	...
	const QSharedPointer<qrtext::core::ast::Node> &)
{
	...
	pushResult(node,
		readTemplate("tableConstructor.t")
	...
	const QSharedPointer<qrtext::core::ast::Node> &)
{
	...
	const qrtext::lua::ast::Identifier *idNode =
		dynamic_cast<qrtext::lua::ast::Identifier *>(node->function().data());
	const QString reservedFunctionCall =
		idNode ? mReservedFunctionsConverter.convert(idNode->name(), arguments) : QString();
