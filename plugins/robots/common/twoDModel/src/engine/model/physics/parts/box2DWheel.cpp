...
Box2DWheel::Box2DWheel(twoDModel::model::RobotModel *const robotModel, Box2DPhysicsEngine *engine, b2Vec2 positionBox2D,
	b2Rot rotationBox2D, Box2DRobot &robot)
	...
	, mPolygon(new b2Vec2[8])
{
	...
	fixtureDef.density = static_cast<float>(
		engine->computeDensity(QPolygonF(QRectF(0, 0, robotModel->parameters()->wheelDiameter() / 2,
...
void Box2DWheel::keepConstantSpeed(float speed)
{
	if (qAbs(prevSpeed - speed) > FLT_EPSILON) {
		mRobot.applyForceToCenter(b2Body_GetWorldVector(mBodyId,
						  b2Vec2 {0.1f * static_cast<float>(mathUtils::Math::sign(speed)), 0}),
			true);
		prevSpeed = speed;
	}
	...
	auto currentForwardSpeed = b2Dot(b2Body_GetLinearVelocity(mBodyId), b2Body_GetWorldVector(mBodyId, {1, 0}));
