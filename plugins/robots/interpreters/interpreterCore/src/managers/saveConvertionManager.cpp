...
void SaveConvertionManager::reconnectEdges(const Id &newBlock, const Id &block, details::ModelsAssistInterface &api)
{
...
{
	return {before300Alpha1Converter(), from300Alpha4to300Alpha5Converter(), from300Beta2to300rc1Converter(),
		from300to301Converter(), from301to302Converter(), from302to310Converter(), from312to313Converter(),
		from320to330Converter(), from330to20204Converter(), from20204to20205Converter(),
...
ProjectConverter SaveConvertionManager::from300Alpha4to300Alpha5Converter()
{
	return constructConverter("3.0.0-a4", "3.0.0-a5",
		{replace({{"JA1", "A1"}, {"JA2", "A2"}, {"JA3", "A3"}, {"JA4", "A4"}, {"JA5", "A5"}, {"JA6", "A6"}
			...
			,
			{"JM1", "M1"}, {"JM2", "M2"}, {"JM3", "M3"}, {"JM4", "M4"}

			,
			{"JB1", "B1"}, {"JB2", "B2"}, {"JB3", "B3"}, {"JB4", "B4"}

			,
			{"JE1", "E1"}, {"JE2", "E2"}, {"JE3", "E3"}, {"JE4", "E4"}

			,
			{"JC1", "C1"}, {"JC2", "C2"}, {"JC3", "C3"}

			,
			{"sensor1", "sensorA1"}, {"sensor2", "sensorA2"}, {"sensor3", "sensorA3"},
			{"sensor4", "sensorA4"}, {"sensor5", "sensorA5"}, {"sensor6", "sensorA6"}

			,
			{"digitSensor1", "sensorD1"}, {"digitSensor2", "sensorD2"}})},
		{}, [](const Id &block) { return block.element().startsWith("Trik"); });
}

ProjectConverter SaveConvertionManager::from300Beta2to300rc1Converter()
{
	return constructConverter("3.0.0-b2", "3.0.0-rc1", {quote("TrikSay", "Text"), quote("PrintText", "PrintText")});
}

qReal::ProjectConverter SaveConvertionManager::from300to301Converter()
{
	return constructConverter("3.0.0", "3.0.1",
		{replace({{"enterButton", "buttonEnter"}, {"escapeButton", "buttonEscape"},
			 {"leftButton", "buttonLeft"}, {"rightButton", "buttonRight"}, {"backButton", "buttonBack"},
			 {"downButton", "buttonDown"}, {"enterButton", "buttonEnter"}, {"upButton", "buttonUp"},
		...
		return false;
			...
			deleteBlocks({"Ev3WaitForUp", "Ev3WaitForEnter", "Ev3WaitForDown", "Ev3WaitForRight",
				"Ev3WaitForLeft", "Ev3WaitForBack"

				,
				"NxtWaitForEnter", "NxtWaitForEscape", "NxtWaitForLeft", "NxtWaitForRight"

				,
				"TrikWaitForEnter", "TrikWaitForLeft", "TrikWaitForRight", "TrikWaitForDown",
				"TrikWaitForUp", "TrikWaitForPower"})});
}
...
{
	const QMap<QString, QString> replacementRules = {{"interpreterBase", "kitBase"},
		{"commonTwoDModel", "twoDModel"}, {"nxtKitInterpreter", "nxt"}, {"ev3KitInterpreter", "ev3"},
		{"trikKitInterpreter", "trik"}, {"NxtRealRobotModel", "NxtUsbRealRobotModel"},
		{"nxtKitRobot", "nxtKitUsbRobot"}, {"TrikRealRobotModelV6", "TrikRealRobotModel"},
		{"lineSensorX", "lineSensor[0]"}, {"lineSensorSize", "lineSensor[1]"},
...
{
	const QMap<QString, QString> replacementRules = {{"gyroscopeX", "gyroscope[0]"}, {"gyroscopeY", "gyroscope[1]"},
		{"gyroscopeZ", "gyroscope[2]"}, {"accelerometerX", "accelerometer[0]"},
		{"accelerometerY", "accelerometer[1]"}, {"accelerometerZ", "accelerometer[2]"}};
...
ProjectConverter SaveConvertionManager::from20204to20205Converter()
{
	return ProjectConverter(editor(), Version::fromString("2020.4"), Version::fromString("2020.4.1"),
		...
		if (!logicalApi.logicalRepoApi()
				.metaInformation("lastKitId")
				.toString()
				.contains("trik", Qt::CaseInsensitive)) {
			return ProjectConverter::NoModificationsMade;
		}
		...
		for (const Id &block : elementsOfRobotsDiagrams(logicalApi)) {
			const Id logicalBlock =
				graphicalApi.isGraphicalId(block) ? graphicalApi.logicalId(block) : block;
			...
			if (graphicalBlock.element() == "PrintText") {
				const auto newType =
					Id(graphicalBlock.editor(), graphicalBlock.diagram(), "TrikPrintText");
				const auto newGraphicalBlock = graphicalApi.createElement(
					graphicalApi.parent(graphicalBlock), newType.sameTypeId(), false,
					graphicalApi.name(graphicalBlock), graphicalApi.position(graphicalBlock),
				...
				while (iterator.hasNext()) {
					iterator.next();
					logicalApi.mutableLogicalRepoApi().setProperty(newLogicalBlock, iterator.key(),
						iterator.value());
				}
...
ProjectConverter SaveConvertionManager::from20205to20222Converter()
{
	return ProjectConverter(editor(), Version::fromString("2020.5"), Version::fromString("2022.2"),
		...
		if (!worldModel.setContent(xml))
			return ProjectConverter::SaveInvalid;
...
qReal::ProjectConverter SaveConvertionManager::constructConverter(const QString &oldVersion, const QString &newVersion,
	...
	const SaveConvertionManager::GraphicalConstructor &constructor)
{
	return [replacer, constructor](const Id &block, GraphicalModelAssistInterface &graphicalApi) {
		...
		graphicalApi.createElement(graphicalApi.parent(block), newBlock, false, graphicalApi.name(block),
			graphicalApi.position(block), graphicalApi.logicalId(block));
		// And initializing it...
