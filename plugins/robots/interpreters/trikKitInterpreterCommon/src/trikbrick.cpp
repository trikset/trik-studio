...
trikControl::MotorInterface *TrikBrick::motor(const QString &port)
{
	...
	if (!mMotors.contains(port)) {
		auto *motor = RobotModelUtils::findDevice<robotParts::Motor>(*mTwoDRobotModel, port);
		if (motor == nullptr) {
...
trikControl::SensorInterface *TrikBrick::sensor(const QString &port)
{
	...
	if (!mSensors.contains(port)) {
		auto *sens = RobotModelUtils::findDevice<robotParts::ScalarSensor>(*mTwoDRobotModel, port);
		if (sens == nullptr) {
...
trikControl::EncoderInterface *TrikBrick::encoder(const QString &port)
{
	...
	if (!mEncoders.contains(port)) {
		auto *enc = RobotModelUtils::findDevice<robotParts::EncoderSensor>(*mTwoDRobotModel, port);
		if (enc == nullptr) {
