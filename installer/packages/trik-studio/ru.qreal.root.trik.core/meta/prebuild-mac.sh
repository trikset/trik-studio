...
cp -fpR $BIN_DIR/libPythonQt{,_QtAll}-Qt*-Python*.dylib                                  	      "$BUNDLE_CONTENTS/Lib"
...
find "$BUNDLE_CONTENTS/Lib" -depth 1 \( -name "libtrik*.dylib" -or -name "libPythonQt*.dylib" \) -print0 | xargs -0 -n 1 -I {} bash -c 'fix_qreal_dependencies "{}"'
