...
cp -pr $BIN_DIR/libPythonQt{,_QtAll}-Qt*-Python*.so*                                        $PWD/../data/lib/
ldd $BIN_DIR/libPythonQt_QtAll-Qt5*-Python3*.so.1.0  | grep python3 | cut -d ' ' -f 3 | head -n 1 | xargs cp -prvt $PWD/../data/lib/
