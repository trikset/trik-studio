...
if [ -d "$TRIK_STUDIO_INSTALL_DIR" ]; then
  ...
  env TRIK_STUDIO_INSTALL_DIR="$TRIK_STUDIO_INSTALL_DIR" "$I" --verbose --no-force-installations --show-virtual-components --script "$SCRIPT_DIR/trik_studio_installscript.qs"
fi
