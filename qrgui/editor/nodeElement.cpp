...
AbstractCommand *NodeElement::changeParentCommand(const Id &newParent, QPointF position) const
{
	...
	auto *changeParentToSceneCommand = new ChangeParentCommand(mLogicalAssistApi, mGraphicalAssistApi, false, id(),
		oldParent, evScene->rootItemId(), oldPos, oldScenePos);
	AbstractCommand *translateCommand = ResizeCommand::create(this, mContents, position, mContents, oldScenePos);
	auto *result = new ChangeParentCommand(mLogicalAssistApi, mGraphicalAssistApi, false, id(),
