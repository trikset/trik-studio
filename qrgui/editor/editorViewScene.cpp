	...
	CreateElementsCommand **createCommandPointer, bool executeImmediately)
{
	...
	if (mEditorManager.elementType(innerElementInfo.id()).type() == ElementType::Type::pattern) {
		...
		CreateAndUpdatePatternCommand *const createGroupCommand =
			new CreateAndUpdatePatternCommand(*this, mModels, {innerElementInfo});
	...
	bool executeImmediately)
{
	...
	if (executeImmediately) {
		if (element.isEdge()) {
			...
			commands::InsertIntoEdgeCommand *insertCommand =
				new commands::InsertIntoEdgeCommand(*this, mModels, Id(), Id(), element.parent(),
					element.position(), QPointF(size.width(), size.height()),
