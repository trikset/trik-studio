...
void MainWindow::openShapeEditor(const QPersistentModelIndex &index, int role, const QString &propertyValue,
	bool useTypedPorts)
{
	auto *shapeEdit = new ShapeEdit(dynamic_cast<models::details::LogicalModel *>(models().logicalModel()), index,
		role, useTypedPorts);
...
void MainWindow::arrangeElementsByDotRunner(const QString &algorithm, const QString &absolutePathToDotFiles)
{
	...
	auto *runner = new DotRunner(diagramId, models().graphicalModelAssistApi(), models().logicalModelAssistApi(),
		editorManager(), absolutePathToDotFiles);
