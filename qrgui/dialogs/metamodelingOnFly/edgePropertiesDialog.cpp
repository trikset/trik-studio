...
EdgePropertiesDialog::EdgePropertiesDialog(const Id &diagram, const EditorManagerInterface &editorManagerProxy,
	QWidget *parent)
	: QDialog(parent)
...
void EdgePropertiesDialog::okButtonClicked()
{
	if (mUi->nameEdit->text().isEmpty()) {
		...
		const IdList edgesWithTheSameNameList =
			mEditorManagerProxy.elementsWithTheSameName(mDiagram, mUi->nameEdit->text(), "MetaEntityEdge");
		if (!edgesWithTheSameNameList.isEmpty()) {
			mEdgeName = mUi->nameEdit->text() + "_" + edgesWithTheSameNameList.count();
			mRestoreElementDialog =
				new RestoreElementDialog(this, mEditorManagerProxy, edgesWithTheSameNameList);
...
{
	mEditorManagerProxy.addEdgeElement(mDiagram, mEdgeName, mUi->nameEdit->text(), mUi->labelTextEdit->text(),
		mUi->labelTypeComboBox->currentText(), mUi->lineTypeComboBox->currentText(),
		mUi->beginTypeComboBox->currentText(), mUi->endTypeComboBox->currentText());
