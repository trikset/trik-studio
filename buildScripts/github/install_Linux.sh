...
if [ "$ID" = "altlinux" ]; then
  ...
  yum update -y && yum install -y --setopt=install_weak_deps=False epel-release
  yum install --allowerasing -y --setopt=install_weak_deps=False sudo which libusbx-devel curl wget make gcc-toolset-"$GCC_VERSION"-{gcc-c++,libasan-devel,libubsan-devel,gdb} \
  ...
  yum install -y --setopt=install_weak_deps=False libxkbcommon-x11 qt5-qtbase-gui #no desire to enumerate all required libraries for QtIFw
  yum install -y --setopt=install_weak_deps=False pulseaudio-libs-glib2 # to run TS and 2D-model even with `minimal` platform
  yum install -y --setopt=install_weak_deps=False qt5-qtscript-devel qt5-qttools-devel qt5-qtmultimedia-devel qt5-qtserialport-devel qt5-qtsvg-devel qt5-qtbase-devel qt5-qtbase-private-devel \
  qt5-qtwayland
