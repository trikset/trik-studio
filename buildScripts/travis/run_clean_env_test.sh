...
case $(uname -s) in
  Linux)
    ...
    ;;
  Darwin)
    env TRIK_STUDIO_INSTALL_DIR="$HOME/T S" ./installer/install_studio_mac.sh
    env QT_QPA_PLATFORM=minimal python3.7 ./buildScripts/tests/fieldstest.py "$HOME/T S/TRIK Studio.app/Contents/MacOS/2D-model" testing
    ;;
  *) exit 1 ;;
esac
