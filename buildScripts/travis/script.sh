...
&& sh -c \"export ASAN_OPTIONS='$([[$TRAVIS_OS_NAME == linux ]] && echo detect_leaks=1 || :) detect_stack_use_after_return=1 fast_unwind_on_malloc=0' LSAN_OPTIONS='suppressions=lsan.supp fast_unwind_on_malloc=1' DISPLAY=:0 && cd bin/$CONFIG && $TESTS\""
