...
{
	return (str[i] != 'L') && (str[i] != 'C') && (str[i] != 'M') && (str[i] != 'Z') && (i != str.length());
}
...
void SdfRenderer::parsestyle(QDomElement &element)
{
	...
	if (!elem.isNull()) {
		...
		if (elem.hasAttribute("fill")) {
			...
			toGen << "\tbrush.setColor(QColor(" << red << ", " << green << ", " << blue << "));\n";
		}
...
void SdfIconEngineV2::paint(QPainter * /*painter*/, const QRect & /*rect*/, QIcon::Mode /*mode*/,
	QIcon::State /*state*/)
{
}
