...
Editor *XmlCompiler::loadXmlFile(const QDir &currentDir, const QString &inputXmlFileName)
{
	...
	if (mEditors.contains(fullFileName)) {
		...
		QDomDocument inputXmlDomDocument =
			xmlUtils::loadDocument(fullFileName, &errorMessage, &errorLine, &errorColumn);
...
void XmlCompiler::generateIncludes(OutFile &out)
{
	...
	const QString dependencies =
		extendedMetamodel.isEmpty() ? QString() : utils::StringUtils::wrap(extendedMetamodel);

	out() << mPluginName << "Plugin::" << mPluginName << "Plugin()\n"
	      << "\t: mMetamodel(nullptr)"
	      << "{\n"
	      << "}\n"
	      << "\n"
	...
	for (const Diagram *diagram : mEditors[mCurrentEditor]->diagrams()) {
		for (const Type *type : diagram->types()) {
			if (const auto *graphicType = dynamic_cast<const GraphicType *>(type)) {
				generateLinks(out, graphicType, graphicType->immediateParents(),
					"generalizationLinkType", false);
				generateLinks(out, graphicType, graphicType->containedTypes(), "containmentLinkType",
					true);
...
void XmlCompiler::generateLinks(OutFile &out, const Type *from, const QStringList &to, const QString &linkType,
	bool areNamesNormalized)
{
	for (const QString &toTypeName : to) {
		const Type *toType = areNamesNormalized ? mEditors[mCurrentEditor]->findTypeByNormalizedName(toTypeName)
		                                        : mEditors[mCurrentEditor]->findType(
								  from->diagram()->name() + "::" + toTypeName);
...
void XmlCompiler::generatePaletteGroupsLists(utils::OutFile &out)
{
	...
	for (const Diagram *const diagram : mEditors[mCurrentEditor]->diagrams().values()) {
		...
		for (const QPair<QString, QStringList> &group : paletteGroups) {
			...
			const QString groupParameters =
				QString(R"("%1", QObject::tr("%2"))").arg(diagramName, groupName);
			out() << "\tmMetamodel->appendDiagramPaletteGroup(" << groupParameters << ");\n";

			for (const QString &name : group.second) {
				out() << "\tmMetamodel->addElementToDiagramPaletteGroup(" << groupParameters
				      << ", QString::fromUtf8(\"" << NameNormalizer::normalize(name) << "\"));\n";
			}
		}
	}
	out() << "}\n\n";
	...
	for (const Diagram *const diagram : mEditors[mCurrentEditor]->diagrams().values()) {
		...
		for (const QString &groupName : paletteGroupsDescriptions.keys()) {
			...
			if (!descriptionName.isEmpty()) {
				out() << "\tmMetamodel->setDiagramPaletteGroupDescription(" << groupParameters
				      << ", QObject::tr(\"" << descriptionName << "\"));\n";
			}
		}
	}
...
void XmlCompiler::generateShallPaletteBeSorted(utils::OutFile &out)
{
	...
	for (const Diagram *const diagram : mEditors[mCurrentEditor]->diagrams().values()) {
		const QString diagramName = NameNormalizer::normalize(diagram->name());
		out() << "\tmMetamodel->setPaletteSorted(QString::fromUtf8(\"" << diagramName << "\"), "
		      << (diagram->shallPaletteBeSorted() ? "true" : "false") << ");\n";
	}
...
void XmlCompiler::generateExplosionsMappings(OutFile &out, const GraphicType *graphicType)
{
	...
	for (const QString &target : explosions.keys()) {
		out() << QString("\tmMetamodel->addExplosion(mMetamodel->elementType(\"%1\", \"%2\")"
				 ...
				 .arg(graphicType->diagram()->name(), graphicType->name(),
...
void XmlCompiler::generateEnumValues(OutFile &out)
{
	...
	std::sort(enumTypes.begin(), enumTypes.end(), [=](EnumType *a, EnumType *b) { return a->name() < b->name(); });
	...
	for (auto &&type : enumTypes) {
		...
		out() << "\tmMetamodel->setEnumEditable(\"" << name << "\", " << (type->isEditable() ? "true" : "false")
		      << ");\n";
	}
