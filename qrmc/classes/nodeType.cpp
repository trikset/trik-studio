...
NodeType::NodeType(const Diagram &diagram, const qrRepo::LogicalRepoApi &api, const qReal::Id &id,
...
QString NodeType::generateNodeClass(const QString &classTemplate)
{
	...
	QString border = isAction == "true" ? compiler.getTemplateUtils(nodeValidBorderTag)
...
void NodeType::generateContainerStuff(QString &classTemplate) const
{
	...
	for (const Id &child : children) {
		...
		if (child.element() == metaEntityPropertiesAsContainer) {
			foundChild = true;
			const QString movableChildren =
				mApi.stringProperty(child, "banChildrenMove") == "true" ? "false" : "true";
			classTemplate.replace(sortingContainerTag, loadBoolProperty(child, "sortContainer"))
				...
				.replace(childrenForestallingSizeTag,
					loadIntProperty(child, "childrenForestallingSize"));
			break;
		}
	}
