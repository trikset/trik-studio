...
void InterpreterTest::SetUp()
{
	...
	ON_CALL(mConfigurationInterfaceMock, devices())
		.WillByDefault(Return(QList<kitBase::robotModel::robotParts::Device *>()));
	...
	ON_CALL(mModel, connectToRobot())
		.WillByDefault(Invoke(&mModelManager, &RobotModelManagerInterfaceMock::emitConnected));
	EXPECT_CALL(mModel, connectToRobot()).Times(AtLeast(0));

	ON_CALL(mModel, disconnectFromRobot())
		.WillByDefault(Invoke(&mModelManager, &RobotModelManagerInterfaceMock::emitDisconnected));
	...
	ON_CALL(mModel, applyConfiguration())
		.WillByDefault(Invoke(&mModelManager, &RobotModelManagerInterfaceMock::emitAllDevicesConfigured));
	...
	mBlocksFactory.configure(mQrguiFacade->graphicalModelAssistInterface(),
		mQrguiFacade->logicalModelAssistInterface(), mModelManager,
		*mQrguiFacade->mainWindowInterpretersInterface().errorReporter(), *mParser);
	...
	ON_CALL(mBlocksFactoryManager, enabledBlocks(_))
		.WillByDefault(Invoke([this](kitBase::robotModel::RobotModelInterface const &robotModel) {
		Q_UNUSED(robotModel)
		return mBlocksFactory.providedBlocks().toSet();
	}));
	EXPECT_CALL(mBlocksFactoryManager, enabledBlocks(_)).Times(0);

	mInterpreter.reset(new BlockInterpreter(mQrguiFacade->graphicalModelAssistInterface(),
		mQrguiFacade->logicalModelAssistInterface(), mQrguiFacade->mainWindowInterpretersInterface(),
		mQrguiFacade->projectManagementInterface(), mBlocksFactoryManager, mModelManager, *mParser));
