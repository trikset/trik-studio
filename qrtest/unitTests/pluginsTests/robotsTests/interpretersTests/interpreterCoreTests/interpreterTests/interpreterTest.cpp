...
void InterpreterTest::SetUp()
{
	...
	DummyBlockFactory blocksFactory;
	...
	ON_CALL(mBlocksFactoryManager, block(_, _)).WillByDefault(
			Invoke([&blocksFactory] (qReal::Id const &id, kitBase::robotModel::RobotModelInterface const &robotModel) {
					Q_UNUSED(robotModel)
	...
	ON_CALL(mBlocksFactoryManager, enabledBlocks(_)).WillByDefault(
			Invoke([&blocksFactory] (kitBase::robotModel::RobotModelInterface const &robotModel) {
					Q_UNUSED(robotModel)
