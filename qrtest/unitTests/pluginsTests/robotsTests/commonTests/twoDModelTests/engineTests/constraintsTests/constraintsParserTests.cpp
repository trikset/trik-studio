...
TEST_F(ConstraintsParserTests, comparisonTest)
{
	auto testSign = [this](const QString &sign, const QString &type) {
		mEvents.clear();
		const QString xml = QString("<constraints>"
					    "	<timelimit value=\"2000\"/>"
...
TEST_F(ConstraintsParserTests, constraintTagAndTypeOfTagTest)
{
	auto testCase = [this](bool checkOnce, const QString &objectId, const QString &type) {
		...
		const QString xml = QString("<constraints>"
					    ...
					    "	</constraint>"
					    "</constraints>")
		                            .arg(checkOnce ? "true" : "false", objectId, type);
		ASSERT_TRUE(mParser.parse(xml));
		ASSERT_EQ(mEvents.count(), 2);
...
TEST_F(ConstraintsParserTests, setObjectStateTest)
{
	...
	const QString increment11Xml = "<sum>"
...
TEST_F(ConstraintsParserTests, extendedSyntaxSetObjectStateTest)
{
	...
	const QString increment11Xml = R"(<sum _1="${object.otherObject.intProperty}" _2="11"/>)";
	testCase("object.otherObject", "intProperty", increment11Xml);
	ASSERT_EQ(objB->otherObject()->intProperty(), 100511);
	testCase("object.otherObject", "stringProperty", "<string value=\"abc123\"/>");
	ASSERT_EQ(objB->otherObject()->stringProperty(), "abc123");
...
TEST_F(ConstraintsParserTests, objectsSetTest)
{
	auto testCase = [this](const QString &object, int size, int first, int last) {
		...
		const QString xml =
			QString("<constraints>"
				...
				"	</event>"
				"</constraints>")
				.arg(object, QString::number(size), QString::number(first), QString::number(last));
		ASSERT_TRUE(mParser.parse(xml));
		ASSERT_EQ(mEvents.count(), 2);
		auto &event = mEvents["event"];
		ASSERT_NE(event, nullptr);
	...
	testCase("object.intListProperty", intListProperty.size(), intListProperty.first(), intListProperty.last());
}
...
TEST_F(ConstraintsParserTests, boundingRectTest)
{
	auto testCase = [this](const QString &prefix, int x, int y, int width, int height) {
		...
		const QString xml = QString("<constraints>"
					    ...
					    "	</event>"
					    "</constraints>")
		                            .arg(prefix, QString::number(x), QString::number(y), QString::number(width),
	...
	const QList<CanvasObject *> shapes = {new EllipseObject(QPoint(0, 0), 50, 50),
		new RectangleObject(QRect(0, 0, 200, 200)), new LineObject(QPoint(-120, -130), QPoint(-80, -70)),
		new PointObject(201, 201)};
	const QRect correctRect(QPoint(-120, -130), QPoint(201, 201));

	ObjectsSet<CanvasObject *> set(shapes);
	mObjects["items"] = &set;
	testCase("objectState object", correctRect.left(), correctRect.top(), correctRect.width(),
		correctRect.height());
	...
	testCase("variableValue name", correctRect.left(), correctRect.top(), correctRect.width(),
		correctRect.height());
	qDeleteAll(shapes);
}
